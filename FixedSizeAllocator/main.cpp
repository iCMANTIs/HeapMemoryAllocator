...
#include <crtdbg.h>
#endif // _DEBUG
FixedSizeAllocator allocator(1, 1024 * 1024);
...
static void* __cdecl malloc(size_t i_size)
{
#ifdef _DEBUG
	printf("malloc %zu\n", i_size);
#endif
	allocator.set(i_size);
	void* ptr = allocator.alloc();
	return ptr;
}

static void __cdecl free(void* i_ptr)
{
#ifdef _DEBUG
...
bool MemorySystem_UnitTest()
{
	...
	if (!AllocatedAddresses.empty())
	{
		...
		while (!AllocatedAddresses.empty())
		{
			void* pPtrToFree = AllocatedAddresses.back();
			AllocatedAddresses.pop_back();
			//printf("%x   %x\n", allocator.getbuffer(), pPtrToFree);
			delete[] pPtrToFree;
		}
